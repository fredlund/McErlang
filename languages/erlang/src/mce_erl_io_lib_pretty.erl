...
pp_fields_tail([{_, Len} = F| Fs], Col0, Col, Ll, TInd, Ind, LD, W) ->
    ...
    ELen = 1 + Len,
    if LD1 =:= 0, ELen + 1 < Ll - Col, W + ELen + 1 =< (?MAXCS), ?ATM_FLD(F);
...
pp_tail([{_, Len} = E| Es], Col0, Col, Ll, TInd, Ind, LD, S, W) ->
    ...
    ELen = 1 + Len,
    if LD1 =:= 0, ELen + 1 < Ll - Col, W + ELen + 1 =< (?MAXCS), ?ATM(E);
...
pp_element({_, Len} = E, Col, Ll, _TInd, _Ind, LD, W)
    when
      Len < Ll - Col - LD, Len + W + LD =< (?MAXCS), ?ATM(E) ->
    {write(E), Len};
...
print_length(R, D, RF) when is_atom(element(1, R)), 
                            is_function(RF) ->
    case RF(element(1, R), tuple_size(R) - 1, R) of
        no -> 
            print_length_tuple(R, D, RF);
        {ok,RDefs,NewR} ->
            print_length_record(NewR, D, RF, RDefs)
    end;
...
printable_list1([C | Cs], N) when is_integer(C), C >= $ , C =< $ÿ ->
    printable_list1(Cs, N - 1);
...
cind_tag_tuple([{_Tag, Tlen}| L], Col, Ll, Ind, LD, W) ->
    ...
    if Ind > 0, TagInd > Ind ->
	   Col1 = Col + Ind,
	   if (?MAXCS) + Col1 =< Ll; Col1 =< Ll div 2 ->
